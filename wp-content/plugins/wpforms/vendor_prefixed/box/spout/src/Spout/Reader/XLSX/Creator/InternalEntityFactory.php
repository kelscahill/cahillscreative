...
namespace WPForms\Vendor\Box\Spout\Reader\XLSX\Creator;
...
use WPForms\Vendor\Box\Spout\Reader\Common\XMLProcessor;
use WPForms\Vendor\Box\Spout\Reader\Wrapper\XMLReader;
use WPForms\Vendor\Box\Spout\Reader\XLSX\Manager\SharedStringsManager;
use WPForms\Vendor\Box\Spout\Reader\XLSX\RowIterator;
use WPForms\Vendor\Box\Spout\Reader\XLSX\Sheet;
use WPForms\Vendor\Box\Spout\Reader\XLSX\SheetIterator;
/**
...
class InternalEntityFactory implements InternalEntityFactoryInterface
{
    ...
    private function createRowIterator($filePath, $sheetDataXMLFilePath, $optionsManager, $sharedStringsManager)
    {
        ...
        $cellValueFormatter = $this->helperFactory->createCellValueFormatter($sharedStringsManager, $styleManager, $shouldFormatDates, $shouldUse1904Dates);
        $shouldPreserveEmptyRows = $optionsManager->getOption(Options::SHOULD_PRESERVE_EMPTY_ROWS);
        return new RowIterator($filePath, $sheetDataXMLFilePath, $shouldPreserveEmptyRows, $xmlReader, $xmlProcessor, $cellValueFormatter, $rowManager, $this);
    }
    /**
     * @param Cell[] $cells
     * @return Row
