...
class Builder {
	...
	/**
	 * Update restrictions for the camera field.
	 *
	 * @since 1.9.8
	 *
	...
	private function process_field_restriction( int $form_id, int $field_id, array $field ): void {
		...
		if ( $type !== 'camera' || empty( $field['is_restricted'] ) ) {
			return;
		}
	...
	/**
	 * Check if the field is a camera field.
	 *
	 * @since 1.9.8
	 *
	 * @param array $field Field data.
	 ...
	 * @return bool
	 */
	private function is_camera_field( array $field ): bool {

		return ! empty( $field['type'] ) && $field['type'] === 'camera';
	}
	...
	private function update_restrictions( int $restriction_id, array $rules, string $password, array $restriction ): void {
		...
		$current_rules = maybe_unserialize( $restriction['rules'] ?? '' );
