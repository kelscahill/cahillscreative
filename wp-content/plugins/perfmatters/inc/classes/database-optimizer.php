<?php
class Perfmatters_Database_Optimizer {
	...
	public function __construct() {
		...
		add_filter('pre_update_option_perfmatters_extras', array($this, 'perfmatters_database_optimization_action'), 10, 2);
	...
	public function init() {
		$this->optimizer = new Perfmatters_Database_Optimization_Process();
	}
	...
	public function perfmatters_database_optimization_action($new_value, $old_value) {
		...
		if(!empty($new_value['optimize_database'])) {
		...
		$new_optimize_schedule = isset($new_value['optimize_schedule']) ? $new_value['optimize_schedule'] : '';
		$old_optimize_schedule = isset($old_value['optimize_schedule']) ? $old_value['optimize_schedule'] : '';
	...
	public function perfmatters_add_database_optimization_cron_schedule($schedules) {
		...
		if(empty($perfmatters_extras['optimize_schedule']) || $perfmatters_extras['optimize_schedule'] == 'daily') {
			return $schedules;
		}

		switch($perfmatters_extras['optimize_schedule']) {
			case 'weekly' :
	...
	public function perfmatters_schedule_database_optimization() {
		$perfmatters_extras = get_option('perfmatters_extras');
		if(!empty($perfmatters_extras['optimize_schedule']) && !wp_next_scheduled('perfmatters_database_optimization')) {
			wp_schedule_event(time(), $perfmatters_extras['optimize_schedule'], 'perfmatters_database_optimization');
		}
	}
	...
	public function perfmatters_run_scheduled_database_optimization() {
		...
		$items = array();
		foreach($optimize_options as $item) {
			if(!empty($perfmatters_extras[$item])) {
				 $items[] = $item;
			}
		}
...
new Perfmatters_Database_Optimizer();