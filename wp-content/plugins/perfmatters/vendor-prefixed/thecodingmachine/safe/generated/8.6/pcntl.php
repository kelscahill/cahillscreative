...
use Perfmatters\Vendor\Safe\Exceptions\PcntlException;
/**
...
function pcntl_getcpuaffinity(?int $process_id = null) : array
{
    \error_clear_last();
    if ($process_id !== null) {
        $safeResult = \Perfmatters\Vendor\pcntl_getcpuaffinity($process_id);
    } else {
        $safeResult = \Perfmatters\Vendor\pcntl_getcpuaffinity();
    }
    if ($safeResult === \false) {
...
function pcntl_setcpuaffinity(?int $process_id = null, array $cpu_ids = []) : void
{
    \error_clear_last();
    if ($cpu_ids !== []) {
        $safeResult = \Perfmatters\Vendor\pcntl_setcpuaffinity($process_id, $cpu_ids);
    } elseif ($process_id !== null) {
        $safeResult = \Perfmatters\Vendor\pcntl_setcpuaffinity($process_id);
    } else {
        $safeResult = \Perfmatters\Vendor\pcntl_setcpuaffinity();
    }
    if ($safeResult === \false) {
        throw PcntlException::createFromPhpError();
    }
