...
namespace Perfmatters\Vendor\Sabberworm\CSS\CSSList;

use Perfmatters\Vendor\Sabberworm\CSS\Comment\CommentContainer;
use Perfmatters\Vendor\Sabberworm\CSS\CSSElement;
...
use Perfmatters\Vendor\Sabberworm\CSS\Property\AtRule;
use Perfmatters\Vendor\Sabberworm\CSS\Property\Charset;
use Perfmatters\Vendor\Sabberworm\CSS\Property\CSSNamespace;
use Perfmatters\Vendor\Sabberworm\CSS\Property\Import;
use Perfmatters\Vendor\Sabberworm\CSS\Property\Selector;
use Perfmatters\Vendor\Sabberworm\CSS\RuleSet\AtRuleSet;
use Perfmatters\Vendor\Sabberworm\CSS\RuleSet\DeclarationBlock;
use Perfmatters\Vendor\Sabberworm\CSS\RuleSet\RuleSet;
use Perfmatters\Vendor\Sabberworm\CSS\Value\CSSString;
use Perfmatters\Vendor\Sabberworm\CSS\Value\URL;
use Perfmatters\Vendor\Sabberworm\CSS\Value\Value;
use function Perfmatters\Vendor\Safe\preg_match;
/**
...
abstract class CSSList implements CSSElement, CSSListItem, Positionable
{
    ...
    private static function parseListItem(ParserState $parserState, CSSList $list)
    {
        ...
        if ($parserState->comes('@')) {
            ...
            if ($atRule instanceof Charset) {
                if (!$isRoot) {
                    throw new UnexpectedTokenException('@charset may only occur in root document', '', 'custom', $parserState->currentLine());
                }
                if (\count($list->getContents()) > 0) {
                    throw new UnexpectedTokenException('@charset must be the first parseable token in a document', '', 'custom', $parserState->currentLine());
                }
    ...
    private static function parseAtRule(ParserState $parserState) : ?CSSListItem
    {
        ...
        if ($identifier === 'import') {
            ...
            $parserState->consumeUntil([';', ParserState::EOF], \true, \true);
            ...
            $result->setAnimationName(\trim($parserState->consumeUntil('{', \false, \true)));
            ...
            if (!($url instanceof CSSString || $url instanceof URL)) {
                throw new UnexpectedTokenException('Wrong namespace url of invalid type', $url, 'custom', $identifierLineNumber);
            }
            return new CSSNamespace($url, $prefix, $identifierLineNumber);
        } else {
            // Unknown other at rule (font-face or such)
            $arguments = \trim($parserState->consumeUntil('{', \false, \true));
    ...
    {
        if (\in_array($sibling, $this->contents, \true)) {
    ...
    {
        $key = \array_search($itemToRemove, $this->contents, \true);
        if ($key !== \false) {
    ...
    {
        $key = \array_search($oldItem, $this->contents, \true);
        if ($key !== \false) {
            if (\is_array($newItem)) {
     ...
     */
    public abstract function isRootList() : bool;
    /**
