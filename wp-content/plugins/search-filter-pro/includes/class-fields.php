...
class Fields {
	...
	public static function init() {
		...
		add_action( 'search-filter/record/set_attributes', array( __CLASS__, 'set_attributes' ), 1, 3 );
	...
	public static function get_field_setting_support( $setting_support, $type, $input_type ) {
		...
		// Add show count to choice fields for custom fields + ACF fields.
			...
			$setting_support['showCount'] = array(
				'conditions' => array(
					...
					'rules'    => array(
						...
						// TODO - move this into the ACF integration class.
						array(
							'option'  => 'dataType',
							'compare' => '=',
							'value'   => 'acf_field',
	 ...
	 * @param    string                        $record_type    The record type to set the attributes for.
	 * @param    \Search_Filter\Records\Record $record    The record to set the attributes for.
	 * @return   array    The attributes to set.
	 */
	public static function set_attributes( $attributes, $record_type, $record ) {

		if ( $record_type !== 'field' ) {
			return $attributes;
		}

		$field_type       = $record->get_attribute( 'type' );
		$field_input_type = $record->get_attribute( 'inputType' );
	...
	public static function register_fields() {
		...
		// Replace the text search field to add specific features.
	...
	public static function register_field_settings() {
		...
		$setting = array(
			...
			'tab'         => 'settings',
			'type'        => 'string',
			// Important - do not use a default, so we can override it on a per field basis when its unset.
			// 'default'     => '',
			'inputType'   => 'Number',
			'min'         => 0,
			'context'     => array( 'admin/field', 'block/field/search', 'admin/field/search', 'admin/field/choice', 'block/field/choice', 'admin/field/range', 'block/field/range', 'admin/field/advanced', 'block/field/advanced', 'admin/field/control', 'block/field/control' ),
	...
	public static function register_custom_field_settings() {
		...
		$setting = array(
			...
			'notice'     => __( 'Enable the indexer to improve performance and enable support for more data types.', 'search-filter-pro' ),
			'group'      => 'data',
			'tab'        => 'settings',
			'type'       => 'string',
	...
	public static function add_custom_field_options( $options, $field ) {
		...
		if ( $custom_field_order === 'alphabetical' ) {
			...
			$order = $wpdb->prepare( ' ORDER BY `meta_value` ' . $query_order );
		} elseif ( $custom_field_order === 'numerical' ) {
			// Allow negatives so cast to signed.
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$order = $wpdb->prepare( ' ORDER BY CAST(`meta_value` AS SIGNED) ' . $query_order );
		}
	...
	public static function field_check_for_indexer_changes( $updated_instance, $section ) {
		...
		if ( self::instance_attribute_will_change( $updated_instance, $previous_attributes, 'multipleMatchMethod' ) ) {
			// Found a changed multipleMatchMethod, so clear the caches for this field.
