<?php
/**
 * Legacy query class for indexer.
 *
...
class Query {
	...
	/**
	 * Whether the current field has match any logic.
	 *
	 * @var bool
	 */
	...
	/**
	 * Whether the query has match any logic.
	 *
	 * @var bool
	 */
	...
	/**
	 * Whether the query has a range field.
	 *
	 * @var bool
	 */
	private $has_range = false;
	...
	/**
	 * Tiered_Cache instance for this query.
	 *
	 * @since 3.2.0
	 *
	 * @var Tiered_Cache|null
	 */
	...
	private function get_cache_instance() {
		if ( null === $this->cache_instance ) {
			...
			$this->cache_instance = new Tiered_Cache(
	...
	public function __construct( $query ) {
		...
		if ( $taxonomy_archive_filter ) {
			...
			// is deperecated, yet they use it for all archives.
		 ...
		 */
		if ( ( $this->query_has_match_any || $this->field_has_match_any || $this->has_range ) && ! empty( $field_cache_args ) ) {
		...
		$collapse_children_into_parents = apply_filters( 'search-filter-pro/indexer/query/collapse_children', false, $this->get_query() );
		if ( $collapse_children_into_parents ) {

			// Convert any object IDs to their parents.
			$updated_post__in = $this->resolve_parents( $updated_post__in );
		}
	...
	private function create_full_query_args( $query_args, $result__post_in, $filtered = true ) {
		...
		if ( $filtered ) {
			...
			// Only set post__in if it's a non-empty array - empty array means "match no posts" in WP_Query.
			// Don't set it at all if null or empty - let WP_Query match all posts matching other criteria.
			if ( is_array( $post__in ) && ! empty( $post__in ) ) {
	...
	private function get_updated_post__in( $query_args, $query_post__in ) {
		$query_args_post__in = isset( $query_args['post__in'] ) ? $query_args['post__in'] : null;

		// If $query_post__in is null, then there are no filters applied to intersect with,
		// use the query args post__in if it exists otherwise return null (don't set post__in).
		...
		if ( $query_args_post__in === null || count( $query_args_post__in ) === 0 ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param array $post__in The post IDs to resolve parents for.
	 * @return array The updated post in.
	 */
	private function resolve_parents( $post__in ) {
		...
		foreach ( $post__in as $post_id ) {
			// Nullish coalesing is marginally faster than using ternary with isset().
			$resolved_post__in[] = $this->object_parents[ $post_id ] ?? $post_id;
		}
	...
	private function get_query_cache( $cache_key ) {
		...
		$tiered_key = 'query_0_' . $cache_key;
		$found      = false;
		$value      = $cache->get( $tiered_key, $found );
	...
	private function add_query_cache( $cache_key, $ids ) {
		...
		$cache->set( $tiered_key, implode( ',', $ids ) );
	}
	 ...
	 * @since 3.0.0
	 *
	 * @param \Search_Filter\Fields\Field $field The field to run the query for.
	 * @return array|null  The result IDs or null if there was no field query.
	 */
	public function field_query( $field ) {

		// `get_query_type()` was removed from the field classes, so we need add in
		// the same logic here to allow the legacy ACF functionaliy to continue to work.
		$query_type  = 'wp_query';
		$index_types = array( 'choice', 'range', 'advanced' );
		if ( in_array( $field->get_attribute( 'type' ), $index_types, true ) ) {
			$query_type = 'indexer';
		}
		$query_type = apply_filters( 'search-filter/indexer/legacy/get_query_type', $query_type, $field );
		if ( $query_type !== 'indexer' ) {
			return null;
		}
	 ...
	 *
	 * @param \Search_Filter\Fields\Field $field The field to run the query for.
	 * @return array|null  The result IDs or null if not found.
	 */
	public function search( $field ) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( 'SELECT CAST(object_id AS UNSIGNED), CAST(object_parent_id AS UNSIGNED) FROM %i WHERE field_id = %d AND value LIKE %s', $table_name, $field_id, '%' . $field_value . '%' ) );
	...
	public function range( $field ) {
		...
		$decimals = 6; // Use our max decimals (set in the UI).
		$decimals = $field->get_attribute( 'rangeDecimalPlaces' );
		...
		// TODO - cast the columns object_id and object_parent_id to an integer in the query itself.
		$this->field_result_ids[ $field_id ] = array();

		$collapse_children_into_parents = apply_filters( 'search-filter-pro/indexer/query/collapse_children', true, $this->query );

		foreach ( $query->items as $item ) {
			$this->field_result_ids[ $field_id ][] = absint( $item->object_id );
			...
			if ( absint( $item->object_parent_id ) !== 0 ) {
				...
				$this->object_parents[ absint( $item->object_id ) ] = absint( $item->object_parent_id );
			}
		}
	...
	public function advanced( $field ) {
		...
		// TODO - casting to int should happen in the query itself.
		$this->field_result_ids[ $field_id ] = array_map( 'absint', $query->items );
	...
	private function get_field_value_ids( $query_id, $field_id, $field_value ) {
		...
		if ( Cache::enabled() ) {
			$tiered_key = 'query_' . $field_id . '_' . $field_value;
			$found      = false;
			$value      = $cache->get( $tiered_key, $found );

			$values = array_map( 'absint', explode( ',', $value ?? '' ) );
			if ( ! $found ) {
				$value = null; // Reset to null to trigger rebuild below.
			}

			if ( $found && $collapse_children_into_parents ) {
				$parents_key    = 'query-parents_' . $field_id . '_' . $field_value;
				$parents_found  = false;
				$result_parents = $cache->get( $parents_key, $parents_found );
				if ( $parents_found && $result_parents ) {
					$result_parents = json_decode( $result_parents, true );
					foreach ( $result_parents as $object_id => $parent_id ) {
						$this->object_parents[ absint( $object_id ) ] = absint( $parent_id );
					}
				}
			}
		}
		...
		if ( $value === null ) {
			...
			for ( $i = 0; $i < $items_count; $i++ ) {
				...
				$values[]           = absint( $item->object_id );
				$query_cache_value .= absint( $item->object_id );
				if ( $i < $items_count - 1 ) {
					$query_cache_value .= ',';
				}
			}
			...
			if ( Cache::enabled() ) {
				$tiered_key = 'query_' . $field_id . '_' . $field_value;
				$cache->set( $tiered_key, $query_cache_value );

				if ( $collapse_children_into_parents ) {
					$parents_key = 'query-parents_' . $field_id . '_' . $field_value;
					$cache->set( $parents_key, wp_json_encode( $query_parents ) );
				}
			}
		}
	 ...
	 *
	 * @param int $field_id The field ID.
	 * @return array|null The field result IDs or null if not found.
	 */
	...
	public function get_combined_result_field_ids_excluding( $exclude_field_id ) {
		...
		if ( is_array( $combined_result_ids ) ) {

			$collapse_children_into_parents = apply_filters( 'search-filter-pro/indexer/query/collapse_children', false, $this->get_query() );

			if ( $collapse_children_into_parents ) {
				// Convert any object IDs to their parents.
				$combined_result_ids = $this->resolve_parents( $combined_result_ids );
				$combined_result_ids = array_unique( $combined_result_ids );
			}
		}
