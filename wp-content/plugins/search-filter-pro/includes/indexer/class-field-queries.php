...
class Field_Queries {
		...
		if ( empty( self::$fields[ $field_id ]['options'] ) ) {
			...
			if ( $count_items ) {
				foreach ( $count_items as $item ) {
					self::$fields[ $field_id ][ $item['value'] ] = $item['count'];
				}
			}
		}
	}
	...
	public static function filter_field_option( $option, $field_id ) {
		...
		if ( $field_id === 0 ) {
			...
			if ( $show_count ) {
				$option['count'] = self::format_count( $count, $show_count_brackets );
			}
			return $option;
		}

		// Use the already stored value from the query.
		if ( isset( self::$fields[ $field_id ][ $option_value ] ) ) {
			$count = self::$fields[ $field_id ][ $option_value ];
			if ( $hide_empty && $count === 0 ) {
				return null;
			}
			if ( $show_count ) {
