 ...
 *
 * Important: This class must not depend on the base plugins as its used in the
 * uninstall process.
 *
...
use Search_Filter_Pro\Core\Exception;
...
class Table_Manager {
	...
	public static function register( $key, $class_name, $should_use = false ) {
		...
		if ( isset( self::$registry[ $key ] ) ) {
			throw new Exception(
				...
				(int) SEARCH_FILTER_PRO_EXCEPTION_TABLE_EXISTS
			);
		}
		...
		if ( ! class_exists( $class_name ) ) {
			throw new Exception(
				...
				(int) SEARCH_FILTER_PRO_EXCEPTION_TABLE_CLASS_MISSING
			);
		}
	...
	public static function update( $key, $class_name ) {
		...
		if ( ! isset( self::$registry[ $key ] ) ) {
			throw new Exception(
				...
				(int) SEARCH_FILTER_PRO_EXCEPTION_TABLE_NOT_FOUND
			);
		}
	 ...
	 *
	 * Instantiates the table if not already cached, then delegates
	 * lifecycle management to the Table instance itself.
	 *
	 * @since 3.2.0
	 *
	...
	public static function flush() {
		...
		// Clear the Table class's registry cache.
	...
	public static function reset() {
		...
		// Flush all caches (instances + registry).
		self::flush();
	}
}
