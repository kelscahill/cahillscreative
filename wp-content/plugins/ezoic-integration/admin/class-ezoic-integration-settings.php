...
class Ezoic_Integration_Admin_Settings {
	...
	public function setup_plugin_options_menu() {
		...
		if ( function_exists( 'is_wpe' ) ) {
			if ( is_wpe() && Ezoic_Integration_Admin::is_cloud_integrated() == false ) {
				$badge_count ++;
			}
		}
...
<?php
		...
		<h2 class="nav-tab-wrapper">
			...
			<?php if ( Ezoic_AdsTxtManager::should_show_adstxtmanager_setting() ) { ?>
...
<?php }
	...
	public function initialize_display_options() {
		...
		if ( $this->ads_enabled ) {
			add_settings_field(
				...
				__( 'Placeholders', 'ezoic' ),
				array( $this, 'placeholders_present_callback' ),
				'ezoic_integration_status',
				'general_settings_section',
				array()
			);
	...
	public function is_integrated_callback() {
		...
		if ( $options['is_integrated'] ) {
			...
			if ( Ezoic_Integration_Admin::is_cloud_integrated() ) {
				$html .= '<p class="text-success"><strong>Active &nbsp;<span class="dashicons dashicons-cloud text-success" title="Cloud Integrated"></span></strong></p>';
			} elseif ( isset( $ezoic_options['disable_wp_integration'] ) && $ezoic_options['disable_wp_integration'] == true ) {
				$html .= '<p class="text-danger"><strong>Disabled &nbsp;<span class="dashicons dashicons-wordpress-alt" title="WordPress Integrated"></span></strong></p>';
			} else {
				$html .= '<p class="text-success"><strong>Active &nbsp;<span class="dashicons dashicons-wordpress-alt text-success" title="WordPress Integrated"></span></strong></p>';
			}
	...
	public function placeholders_present_callback() {
		if ( !$this->ads_enabled ) {
			return;
		}

		$initializing = isset( $_GET['create_default'] ) && $_GET['create_default'];
		...
		if ( !$initializing ) {
			...
			$html .= $publisher_ads->count();

			if ( !$publisher_ads->has_default_ads() ) {
				$html .= ' &nbsp; [<a href="?page=' . EZOIC__PLUGIN_SLUG . '&tab=integration_status&create_default=1">create default placeholders</a>]';
			} else {
				$html .= ' &nbsp; [<a href="https://pubdash.ezoic.com/monetization?scroll=ad_tester" target="_blank">manage placeholders</a>]';
			}
		} else {
			$html .= '<i>Default ads are being created, please check back in a few minutes</i>';
		}
	...
	private function request_data_from_ezoic( $final_content ) {
		$timeout = 5;
		...
		return array( "result" => $this->parse_page_contents( $final ) );
