...
class Ezoic_Microdata_Filters {
    ...
    public function get_template_part( $part, $name ) {
        ...
        if ( in_the_loop() && $type == "content" ) {
            ...
            if ( $wp_query->current_post == 0 && !is_paged() ) {
                ...
                ?>
					</section><section itemscope itemtype="http://schema.org/Article">
				<?php
}
    ...
    private function wrap_html_for_type( $type, $html, $id = null ) {
        if ( $type == 'content' ) {
            if ( $this->is_list_page() ) {
                ...
                return '<section itemprop="schema.org/mainEntityOfPage">' . $html . '</section>';
            } else {
        ...
        if ( $type == 'title' && $post_url ) {
            $meta_url = '<meta itemprop="url" content="' . esc_url( $post_url ) . '">';
            return $html . $meta_url;
        }
    ...
    public function get_comment_author_link( $link ) {
        ...
        $replaces = array(
            '$1$2 fn n$3',
            '$1 itemscope itemprop="url"$2',
            '$1<span itemprop="name">$2<!-- ez_commento_insert --></span>$3',
        );
     ...
     * @since  0.1.0
     */
    public function navigation_markup_template( $output = '', $class = null ) {
    ...
    public function body_output( $input ) {
        ...
        return $input . ' dir=' . $dir . ' itemscope="itemscope" itemid="' . $current_url . '" itemtype="http://schema.org/WebPage" itemref="feed commentsfeed"';
    }
    ...
    public function main_output( $input ) {
        if ( is_singular( 'post' ) || ( is_front_page() && !is_home() ) ) {
            $input = $input . ' itemscope="itemscope" itemtype="http://schema.org/Article" itemprop="mainEntityOfPage" ';
        } elseif ( is_search() ) {
            $input = $input . ' itemscope="itemscope" itemtype="http://schema.org/SearchResultsPage" itemprop="mainContentOfPage" ';
        } elseif ( $this->is_list_page() ) {
            $input = $input . '  itemscope="itemscope" itemtype="http://schema.org/CollectionPage" itemprop="mainEntityOfPage"  ';
        } else {
            $input = $input . ' itemscope="itemscope" itemprop="mainContentOfPage" ';
        }
    ...
    public function modify_head_tag( $output ) {
        $matches = ['/<link (rel="alternate".*?php\/feed\/.*?)>/i',
            '/<link (rel="alternate".*?comments\/feed\/.*?)>/i'];
        $replaces = ['<link id="feed" itemscope itemprop="url" $1>',
            '<link id="commentsfeed" itemscope itemprop="url" $1>'];
		$filtered = preg_replace( $matches, $replaces, $output );
		if ( is_null( $filtered ) ) {
			return $output;
		}
    ...
    public function author_output( $input ) {
        return $input . ' itemprop="author" itemscope="itemscope" itemtype="http://schema.org/Person"';
    }
    ...
    public function author_meta_output( $input ) {
        return $input . ' itemprop="name" content="$3"';
    }
}
