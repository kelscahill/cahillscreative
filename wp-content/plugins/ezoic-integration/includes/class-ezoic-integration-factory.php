...
namespace Ezoic_Namespace {
	if ( ! defined( 'EZOIC_INTEGRATION_VERSION' ) ) {
		define( 'EZOIC_INTEGRATION_VERSION', '2.3.1' ); // update plugin version number
	}
	...
	class Ezoic_Integration_Factory {
		...
		private function new_ezoic_filter() {
			...
			$is_debug = isset($_GET["ez_wp_debug"]) && $_GET["ez_wp_debug"] == "1";
		...
		private function new_ezoic_debug() {
			if ( $this->cache_type != Ezoic_Cache_Type::NO_CACHE ) {
				//echo "we are cache debug";
				return new Ezoic_Integration_Cache_Debug();
			}

			return new Ezoic_Integration_WP_Debug( $this->cache_type );
		}
