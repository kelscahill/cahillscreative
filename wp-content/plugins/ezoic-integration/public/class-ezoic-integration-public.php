...
class Ezoic_Integration_Public {
	...
	public function process_final_content( $content ) {
		...
		$content = $this->modify_body_tag( $content );
		...
		$content = $this->modify_head_tag( $content );
		return $content;
	}
	...
	public function content_filter( $content, $id = null ) {
		if ( $this->is_list_page() ) {
			...
			// Load appropriate content processor
			if ( \in_array( 'dom', \get_loaded_extensions() ) ) {
				$content_processor = new Ezoic_Integration_Content_Processor_DOM( $content );
			} else {
				$content_processor = new Ezoic_Integration_Content_Processor_Regex( $content );
			}

			return $content_processor->render();
		}
	}
	...
	public function title_filter( $title, $id = null ) {
		...
		if ( isset( $post ) ) {
			...
			if ( $next !== '' && $id == $next->ID ) {
				return apply_filters( 'ez_next_post_title', $title, $id );
			}

			if ( $prev !== '' && $id == $prev->ID ) {
				return apply_filters( 'ez_next_previous_post_title', $title, $id );
			}
		}
	...
	private function modify_body_tag( $content ) {
		$top_of_page = apply_filters( 'ez_top_of_page', '' );
		$body_attributes = apply_filters( 'ez_body_attributes', '' );
		$filtered = preg_replace( '/<body(.*)>/', '<body$1' . '$2' . $body_attributes . '>' . $top_of_page, $content );
		if ( is_null( $filtered ) ) {
			return $content;
		}
		return $filtered;
