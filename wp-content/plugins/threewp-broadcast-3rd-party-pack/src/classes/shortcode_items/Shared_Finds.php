	...
	extends \threewp_broadcast\premium_pack\classes\generic_items\Shared_Finds
{
	...
	**/
	public function add_find( $find )
	{
		...
		{
			ThreeWP_Broadcast()->debug( 'Shared finds: adding shortcode %s', $key );

			$col = $collection->collection( 'source' );
			$col->set( 'find', $find );
		}
		else
		{
			$col = $collection->collection( $key )->collection( 'source' );
